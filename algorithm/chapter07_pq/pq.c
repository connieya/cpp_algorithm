...
void PQ_SwapNodes(PriorityQueue * PQ , int child , int parent)
{
    int CopySize = sizeof(PQNode);
    PQNode * Temp = (PQNode *) malloc(CopySize);

    memcpy(Temp , &PQ->Nodes[child] , CopySize);
    memcpy(&PQ->Nodes[child] , &PQ->Nodes[parent] , CopySize);
    memcpy(&PQ->Nodes[parent],Temp,CopySize);
    free(Temp);
}
